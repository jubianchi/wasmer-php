...
$cl = function () { var_dump('Hello from PHP named closure'); return 0; };
...
class Sandbox {
    public function run($store, $module, $functype) {
        $func = wasm_func_new($store, $functype, function () { var_dump('Hello from PHP bound closure'); return $this; });
        $extern = wasm_func_as_extern($func);
        $externs = new Wasm\Vec\Extern([$extern]);
        wasm_instance_new($store, $module, $externs);
    }
